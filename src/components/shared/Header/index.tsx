...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      window.innerWidth > 1024 &&
        (intro!.style.transform = `translateY(${value * 0.5}px)`);
    };
  ...
  return (
    <header
      className={clsx(
        "relative w-full flex justify-center transition-opacity duration-300 sticky z-50 top-0 bg-white/95 motion-reduce:transition-none",
        ubuntu.className,
        { ["opacity-0"]: !visible && !open }
      )}
      id='header'
    >
      <div className='w-full h-16 max-w-6xl mx-8 sm:mx-8 flex justify-between items-center'>
        <div
